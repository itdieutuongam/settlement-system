# app.py - PHIÊN BẢN HOÀN CHỈNH THEO YÊU CẦU "NGƯỜI DUYỆT CUỐI" (02/12/2025)
...
app = Flask(__name__)
app.secret_key = "DTA_QTKT02_BM02_2025_V2"
...
def init_db():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS qtkt_forms (
            ...
            next_approver TEXT,
            final_approver_name TEXT  -- CỘT MỚI: Người duyệt cuối (dự kiến hoặc thực tế)
        )
    ''')

    # Tự động thêm cột nếu chưa tồn tại (an toàn khi chạy lại)
    c.execute('PRAGMA table_info(qtkt_forms)')
    columns = [row[1] for row in c.fetchall()]
    if 'final_approver_name' not in columns:
        c.execute('ALTER TABLE qtkt_forms ADD COLUMN final_approver_name TEXT')
...
def dashboard():
    ...
    c.execute("SELECT * FROM qtkt_forms WHERE next_approver = ? AND status = 'Chờ duyệt' ORDER BY id DESC", (my_full,))
    pending = c.fetchall()
    conn.close()
...
def qtkt_form():
    ...
    if request.method == "POST":
        try:
            ...
            ly_do_tam_ung = request.form["ly_do_tam_ung"].strip()
            ...
            next_approver_name = next_approver_full.split(" - ")[0]  # Tên thuần
    ...
    approvers = [
        f"{v['name']} - {v['department']}"
        for k, v in USERS.items()
        if v["role"] in ["Manager", "BOD"] and k != user["email"]
    ]
    return render_template("qtkt_form.html", user=user, departments=DEPARTMENTS,
                           approvers=approvers, nguon_kinh_phi_list=NGUON_KINH_PHI)
...
def approve(form_id):
    ...
    if request.method == "POST":
        ...
        if decision == "reject":
            # Từ chối → người từ chối là người duyệt cuối thực tế
            c.execute("""
                UPDATE qtkt_forms 
                SET status = 'Từ chối', 
                    next_approver = ?, 
            ...
            if user["role"] == "BOD":
                # BOD duyệt xong → BOD là người duyệt cuối thực tế
                c.execute("""
                    UPDATE qtkt_forms 
                    SET status = 'Đã duyệt', 
                        next_approver = ?, 
                ...
                # Manager chỉ chuyển tiếp → không thay đổi final_approver_name
                ...
                if next_person:
                    ...
                    return render_template("qtkt_approve.html", form=form, user=user, approvers=approvers, is_bod=False)
...
if __name__ == "__main__":
    init_db()
    print("="*60)
    print("HỆ THỐNG QTKT02-BM02 – ĐÃ HOÀN THIỆN TÍNH NĂNG NGƯỜI DUYỆT CUỐI")
